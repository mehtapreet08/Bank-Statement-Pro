...
def init_components():
    ...
    return (pdf_processor, transaction_parser, ai_categorizer, data_manager, 
...
def render_upload_view(pdf_proc, trans_parser, ai_cat, data_mgr, audit_log):
...
def render_transaction_details_view(data_mgr, ai_cat, audit_log):
    ...
    col1, col2 = st.columns(2)
    with col1:
        start_date = st.date_input("From Date", value=pd.to_datetime(df['date'], dayfirst=True).min())
    with col2:
        end_date = st.date_input("To Date", value=pd.to_datetime(df['date'], dayfirst=True).max())
...
def main():
    ...
    # Initialize components
    (pdf_proc, trans_parser, ai_cat, data_mgr, chart_gen, 
    ...
    if selected_view == "Upload & Process":
        render_upload_view(pdf_proc, trans_parser, ai_cat, data_mgr, audit_log)
